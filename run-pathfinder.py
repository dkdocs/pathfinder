...
# def are_on_same_side(line, points):
...
axis_X = gl.Line(gp.Point(0, 0), gp.Point(1, 0))
axis_Y = gl.Line(gp.Point(0, 0), gp.Point(0, 1))
...
def connect_edges(src, dst, edges):
    ...
    if len(src_points) > 1 or len(dst_points) > 1:
        ...
        edge = [convert_pixel_latlong((src, dst))]
...
def graph_walker(current_cell, tower_to_connect, path_matrix, edges: list, level=0):
    ...
    for neighbor in get_path_neighbors(current_cell, path_matrix):
        if neighbor in point_to_grid_map:
            connect_edges(tower_to_connect, neighbor, edges)
